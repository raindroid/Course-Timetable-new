...
def get_artsci_course_detail(course: str, time:str="20219", save_file: str = '', save_file_ori: str = ''):
    """
    ...
    api_url = f'/api/{time}/courses?org=&code={course}&section=&studyyear=&daytime=&weekday=&' \
              'prof=&breadth=&online=&waitlist=&available=&title='

    page = get_page(url + api_url)
    if not page:
        # empty page found
        print(f'Nothing related to {course} found')
        return
...
def download_artsci_table(db: CourseDB, col_name: str, time: str="20219", save_year_course: bool = True, drop_frist: bool = True) -> str:
    if drop_frist:
    ...
    for index, courseName in enumerate(course_names):
        courseData = get_artsci_course_detail(courseName, time=time)
        if courseData:
...
if __name__ == '__main__':
    ...
    # print(get_artsci_course_detail('CSC108H1'))
